  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--card-border)] bg-[var(--background)]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-5">
        <Link href={localizedPath("/", locale)} className="font-display text-xl font-bold tracking-tight text-[var(--foreground)]">
        ...
        <div className="flex items-center gap-2">
          <Link href={localizedPath("/contact", locale)} className="hidden rounded-xl border border-[var(--card-border-strong)] bg-[var(--card-bg-soft)] px-4 py-2 text-sm font-semibold text-[var(--foreground)] transition hover:border-[var(--gold)] hover:text-[var(--gold-light)] md:inline-flex">
            {labelMap[locale].Contact}
          </Link>
