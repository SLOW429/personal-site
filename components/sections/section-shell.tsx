...
function ActionLink({ label, href, rtl = false }: { label: string; href: string; rtl?: boolean }) {
  const external = /^https?:\/\//.test(href);
  const className = rtl
    ? "rounded-2xl bg-gradient-to-l from-[var(--gold-light)] to-[var(--gold)] px-5 py-3 font-semibold text-[#071018] transition hover:-translate-y-0.5"
    : "rounded-2xl bg-gradient-to-r from-[var(--gold-light)] to-[var(--gold)] px-5 py-3 font-semibold text-[#071018] transition hover:-translate-y-0.5";
...
export function SectionShell({ section, content, locale = "en" }: { section: string; content: SectionContent; locale?: "en" | "ar" }) {
  const rtl = locale === "ar";

  return (
    <main dir={rtl ? "rtl" : "ltr"} lang={locale} className="min-h-[70vh] px-5 py-16 md:py-24">
      <section className="mx-auto w-full max-w-6xl">
            ...
            <article key={group.title} className="rounded-3xl border border-[var(--card-border)] bg-[var(--card-bg)] p-6 shadow-[0_0_60px_rgba(126,196,255,0.05)] backdrop-blur-xl">
              ...
              <ul className="mt-5 space-y-3">
                {group.items.map((item) => (
                  <li key={item} className={`${rtl ? "border-r pr-4" : "border-l pl-4"} border-[var(--gold)]/40 text-sm leading-6 text-[var(--muted)]`}>{item}</li>
                ))}
              </ul>
            </article>
          ))}
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          {content.links.map(([label, href]) => <ActionLink key={`${label}-${href}`} label={label} href={href} rtl={rtl} />)}
        </div>
      </section>
    </main>
  );
